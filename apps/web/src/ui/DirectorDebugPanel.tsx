...
type DirectorDebugPanelProps = {
  debugSnapshot: DirectorDebugSnapshot | null
  loading: boolean
}

export function DirectorDebugPanel({ debugSnapshot, loading }: DirectorDebugPanelProps) {
  const [open, setOpen] = useState(false)
  const status = loading
    ? '加载追踪'
    : !debugSnapshot
      ? '暂无追踪'
      : debugSnapshot.resultSource === 'model'
        ? '模型'
        : debugSnapshot.resultSource === 'fallback'
          ? '兜底'
          : debugSnapshot.error
            ? '失败'
            : '已捕获'
      ...
      {open && (
        <div className="strategy-debug-panel">
          <div className="strategy-debug-meta">
            ...
            <span>来源：{formatSource(debugSnapshot?.resultSource)}</span>
          ...
          <DebugBlock title="解析响应" body={formatJson(debugSnapshot?.parsedResponse)} />
